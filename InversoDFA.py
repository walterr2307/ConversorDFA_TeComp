...
class InversoDFA:  # Define a classe que cria o AFD invertido (inverso das transições)
    def __init__(self, arquivo_dfa):
        self.letras = arquivo_dfa.letras  # Guarda o alfabeto do AFD
        self.estados = self.copiarEstados(
            arquivo_dfa.estados
        )  # Guarda os estados do AFD
        self.estado_inicial = (
            arquivo_dfa.estado_inicial
        )  # Guarda o estado inicial original
        self.estados_invertidos = (
            self.gerarEstadosInvertidos()
        )  # Cria estrutura vazia para as transições invertidas
        ...
        with open(
            "AFD Invertido.txt", "w", encoding="utf-8"
        ) as arquivo:  # Escreve o AFD invertido no arquivo
    ...
    def gerarEstadosInvertidos(
        self,
    ):  # Cria uma lista de dicionários vazios, um para cada estado, para armazenar as transições invertidas
        estados = []
    ...
    def inverterFuncoes(self):  # Inverte as transições do AFD original
        for u in self.estados:  # Para cada estado u
            for i in range(len(self.estados)):  # Compara com todos os outros estados
                for letra in self.letras:
                    if (
                        u.regras[letra] == self.estados[i].simbolo
                    ):  # Se a transição leva para o estado atual
                        self.estados_invertidos[i][letra] += (
                            u.simbolo + " "
                        )  # Adiciona u como origem invertida

    def colocarNovasFuncoes(
        self,
    ):  # Atualiza os estados com as novas transições invertidas
    ...
    def colocarFuncoes(
        self,
    ):  # Adiciona as transições e os estados finais/iniciais no arquivo
        for estado in self.estados:
            for letra in estado.regras:
                if estado.regras[letra] != "":
                    lista = estado.regras[letra].split(
                        " "
                    )  # Divide múltiplos destinos (caso de NFA invertido)
    ...
    ):  # Junta os elementos com vírgula e espaço
